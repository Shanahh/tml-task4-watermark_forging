...
def args():
 p=argparse.ArgumentParser();p.add_argument('--dataset',type=Path,required=True);p.add_argument('--category',required=True,choices=CATEGORIES);p.add_argument('--models',type=Path,required=True);p.add_argument('--output-dir',type=Path,default=Path('pgd_candidates'));p.add_argument('--eps-grid',default='0.0039215686,0.0078431373,0.011764706');p.add_argument('--steps',type=int,default=50);p.add_argument('--step-size',type=float,default=.0009803922);p.add_argument('--mse-weight',type=float,default=10.0);p.add_argument('--tv-weight',type=float,default=.05);return p.parse_args()
def hp(x):
...
def main():
 a=args();dev=torch.device('cuda' if torch.cuda.is_available() else 'cpu');ms=load_models(a.models/a.category.lower(),dev);_,clean=load_dataset(a.dataset);lo,hi=CATEGORY_RANGES[a.category]
 for eps in [float(v) for v in a.eps_grid.split(',')]:
  ...
  for i,x in clean.items():save_rgb(optimize(x,ms,eps,a,dev) if lo<=i<=hi else x,out/f'{i}.png')
  print('saved',out)
