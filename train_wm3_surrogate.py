...
from common import load_dataset,high_pass
class DS(Dataset):
...
def args():
 p=argparse.ArgumentParser();p.add_argument('--dataset',type=Path,required=True);p.add_argument('--output-dir',type=Path,default=Path('wm3_surrogates'));p.add_argument('--ensemble-size',type=int,default=5);p.add_argument('--epochs',type=int,default=40);p.add_argument('--batch-size',type=int,default=8);p.add_argument('--lr',type=float,default=1e-3);p.add_argument('--seed',type=int,default=2026);return p.parse_args()
def main():
 a=args();a.output_dir.mkdir(parents=True,exist_ok=True);src,clean=load_dataset(a.dataset);pos=src['WM_3'];neg=[x for x in clean.values() if x.shape[:2]==pos[0].shape[:2]];ims=pos+neg;y=np.r_[np.ones(len(pos),np.float32),np.zeros(len(neg),np.float32)];dev=torch.device('cuda' if torch.cuda.is_available() else 'cpu');meta=[]
 for member in range(a.ensemble_size):
  ...
  for e in range(a.epochs):
   ...
   auc=roc_auc_score(ys,ps);print(member,e+1,auc)
   if auc>best:best=auc;state={k:v.detach().cpu() for k,v in m.state_dict().items()}
  path=a.output_dir/f'wm3_detector_{member}.pt';torch.save({'state_dict':state,'val_auc':best,'seed':seed},path);meta.append({'path':str(path),'val_auc':best,'seed':seed})
